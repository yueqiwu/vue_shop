...
export default {
    ...
    return {
      ...
      addFormRules: {
        ...
        email: [
          ...
          { validator: self.checkEmail, trigger: 'blur' }
        ],
        mobile: [
          { required: true, message: '请输入手机号', trigger: 'blur' },
          { validator: self.checkMobile, trigger: 'blur' }
        ]
      },
      // 控制修改用户对话框的显示与隐藏
  ...
  methods: {
    async getUserList() {
      ...
      if (res.meta.status !== 200) return message.err('获取用户列表失败！')
    ...
    async userStateChanged(userInfo) {
      ...
      const res = await changeUserStateApi(userInfo.id, userInfo.mg_state)
      console.log(res)
    ...
    addUser() {
      this.$refs.addFormRef.validate(async vaild => {
        ...
        if (res.meta.status !== 201) message.error('添加用户失败！')
    ...
    async removeUserById(id) {
      ...
      if (res.meta.status !== 200) {
        return message.error('删除用户失败！' + res.meta.msg)
      }
    ...
    async setRole(userInfo) {
      ...
      // 在展示对话框之前，获取所有角色的列表
      const res = await getRolesApi('roles')
      if (res.meta.status !== 200) {
    ...
    async saveRoleInfo() {
      ...
      const res = await setRolesApi(this.userInfo.id, this.selectedRoleId)

      if (res.meta.status !== 200) {
        return message.error('更新角色失败！' + res.meta.msg)
      }
